...
public class FileOperatorUtil {
	...
	private static FileOperatorUtil fileOperatorUtil = new FileOperatorUtil();
	...
	private synchronized static String createFilePathByRuleOfTime(String fileType,String storagePath){
		...
		return "/" + storagePath + "/" + fileType + "/" + calendar.get(Calendar.YEAR) 
				+ "/" + calendar.get(Calendar.MONTH) + "/" + calendar.get(Calendar.DAY_OF_MONTH) + "/" + System.nanoTime() + "." + fileType;
	}
	 ...
	 * @warn 若存在无法识别的类型，我们会以.yl类型返回
	 */
	public static List<Map<String, Object>> storageFilesWithoutFileType(File[] files, String storagePath,List<String> fileNames) throws Exception{
	 ...
	 * @param filePath 文件路径
	 */
	public static void createFilebyFilePath(String filePath) throws Exception {
		try {
			File file = new File(filePath);
			if (file.exists() && file.isFile()) {
			} else {
	...
	private static void initFileTypeCode() {
		fileTypeCode = new HashMap<String, String>();
		Properties properties = PropertiesReaderUtil
				.getProperties("com/yl/base/file/fileTypeCode.properties");
